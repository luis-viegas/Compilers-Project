...
public class VisitorJasmin extends PreorderJmmVisitor<MySymbolTable, Boolean> {
    ...
    public Boolean visitFunction(JmmNode node, MySymbolTable mySymbolTable) {
        ...
        for(JmmNode child : node.getChildren()) {
            if (child.getKind().equals("ExprStmt") || child.getKind().equals("Assignment") || child.getKind().equals("VarDeclaration")|| child.getKind().equals("IfStatement") || child.getKind().equals("ElseStmt") || child.getKind().equals("WhileStatement") || child.getKind().equals("ReturnExpr")) {
                result.add(child);
            }
        }
